<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="Settlement[@id='village_CHUB13_3']">
		<xsl:copy>
			<xsl:apply-templates select="@*"/>
			<xsl:attribute name='name'>{=TI0M6tgm}Inuyama</xsl:attribute>
			<xsl:apply-templates select="node()"/>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="Settlement[@id='castle_KA2']">
		<xsl:copy>
			<xsl:apply-templates select="@*"/>
			<xsl:attribute name='name'>{=mQQksGbP}Kozukue Castle</xsl:attribute>
			<xsl:apply-templates select="node()"/>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="Settlement[@id='castle_KIN19']">
		<xsl:copy>
			<xsl:apply-templates select="@*"/>
			<xsl:attribute name='name'>{=iYjZ69ZO}Iga-Ueno Castle</xsl:attribute>
			<xsl:apply-templates select="node()"/>
		</xsl:copy>
	</xsl:template>
